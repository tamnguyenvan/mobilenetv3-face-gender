...
class SSD(nn.Module):
    ...
    def compute_loss(self, targets: List[Dict[str, Tensor]], head_outputs: Dict[str, Tensor], anchors: List[Tensor],
        ...
        gender_logits = head_outputs['gender_logits']
        ...
        cls_targets = []
        gender_targets = []
        for (targets_per_image, bbox_regression_per_image, cls_logits_per_image, gender_logits_per_image, anchors_per_image,
             matched_idxs_per_image) in zip(targets, bbox_regression, cls_logits, gender_logits, anchors, matched_idxs):
            ...
            gt_gender_target = torch.zeros((gender_logits_per_image.size(0), ), dtype=targets_per_image['genders'].dtype,
                                            device=targets_per_image['genders'].device)
            gt_gender_target[foreground_idxs_per_image] = \
                targets_per_image['genders'][foreground_matched_idxs_per_image]
            gender_targets.append(gt_gender_target)
        ...
        num_gender_classes = gender_logits.size(-1)
        gender_loss = F.cross_entropy(
            gender_logits.view(-1, num_gender_classes),
            gender_targets.view(-1),
            reduction='none'
        ).view(gender_targets.size())
        ...
        return {
            ...
            'gender_classification': (gender_loss[foreground_idxs].sum() + gender_loss[background_idxs].sum()) / N,
        }
